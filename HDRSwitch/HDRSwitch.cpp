...
static void showError(const std::string& msg)
{
    ...
    MessageBoxW(NULL, wmsg.c_str(), L"HDRSwitch Error", MB_OK | MB_ICONWARNING);
}
...
static DISPLAYCONFIG_MODE_INFO* GetAllDisplayIdentifiers()
{
    ...
    // Return nullptr if anything failed
    return nullptr;
}
...
static UINT32 _GetUID(UINT32 id)
{
	...
	if (ERROR_SUCCESS == GetDisplayConfigBufferSizes(QDC_ONLY_ACTIVE_PATHS, &pathCount, &modeCount))
	{
        ...
        // fallback if buffer sizes fail or allocations fail
        return 0;
	}
    return 0; // fallback if GetDisplayConfigBufferSizes failed
}
...
UINT32 GetPrimaryMonitorUID()
{
	DISPLAY_DEVICE dd;
	dd.cb = sizeof(dd);

	for (UINT32 i = 0; EnumDisplayDevices(nullptr, i, &dd, 0); ++i)
	{
		if (dd.StateFlags & DISPLAY_DEVICE_PRIMARY_DEVICE)
		{
			// _GetUID expects the source ID, usually matches i
			return _GetUID(i);
		}
	}
...
void SetPrimaryHDR(bool enable)
{
	UINT32 primaryUID = GetPrimaryMonitorUID();
	if (primaryUID != 0)
	{
		SetHDR(primaryUID, enable);
	}
	else
	{
		std::cerr << "Failed to find the primary monitor UID.\n";
	}
}
...
extern "C"
{
	...
	__declspec(dllexport) void SetHDRonPrimary(bool enabled)
	{
		SetPrimaryHDR(enabled);
	}
